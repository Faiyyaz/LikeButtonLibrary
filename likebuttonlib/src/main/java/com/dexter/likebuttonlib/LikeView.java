...
public class LikeView extends View {
    ...
    public LikeView(Context context, final LikeButton likeButton, LikeParams likeParams) {
        super(context);


        initShineParams(likeParams, likeButton);


        this.mLikeAnimator = new LikeAnimator(animDuration, shineDistanceMultiple, clickAnimDuration);
        ...
        mLikeAnimator.addListener(new Animator.AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animator) {
            ...
            @Override
            public void onAnimationEnd(Animator animator) {
                likeButton.removeView(LikeView.this);
            }
    ...
    public void showAnimation(LikeButton likeButton) {
        btnWidth = likeButton.getWidth();
        btnHeight = likeButton.getHeight();
        ...
        if (isWindowsNotLimit(likeButton.activity)) {
            likeButton.activity.getWindow().getDecorView().getLocalVisibleRect(visibleFrame);
        } else {
            likeButton.activity.getWindow().getDecorView().getWindowVisibleDisplayFrame(visibleFrame);
        }

        centerAnimX = location[0] + btnWidth / 2 - visibleFrame.left; // If navigation bar is not displayed on left, visibleFrame.left is 0.
        if (isTranslucentNavigation(likeButton.activity)) {
            if (isFullScreen(likeButton.activity)) {
                centerAnimY = visibleFrame.height() - likeButton.getBottomHeight(false) + btnHeight / 2;
            } else {
                centerAnimY = visibleFrame.height() - likeButton.getBottomHeight(true) + btnHeight / 2;
            }
        } else {
            centerAnimY = getMeasuredHeight() - likeButton.getBottomHeight(false) + btnHeight / 2;
        }
        mLikeAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLikeAnimator != null && !isRun) {
            isRun = true;
            showAnimation(mLikeButton);
        }
    }
    ...
    private void initShineParams(LikeParams likeParams, LikeButton likeButton) {
        shineCount = likeParams.shineCount;
        turnAngle = likeParams.shineTurnAngle;
        smallOffsetAngle = likeParams.smallShineOffsetAngle;
        enableFlashing = likeParams.enableFlashing;
        allowRandomColor = likeParams.allowRandomColor;
        shineDistanceMultiple = likeParams.shineDistanceMultiple;
        animDuration = likeParams.animDuration;
        clickAnimDuration = likeParams.clickAnimDuration;
        smallShineColor = likeParams.smallShineColor;
        bigShineColor = likeParams.bigShineColor;
        shineSize = likeParams.shineSize;
        if (smallShineColor == 0) {
            smallShineColor = colorRandom[6];
        }

        if (bigShineColor == 0) {
            bigShineColor = likeButton.getColor();
        }
